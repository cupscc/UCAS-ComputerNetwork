...
#define IP_DF 0x4000 // Do not Fragment 不要分片
struct iphdr
{
...
#define IP_BASE_HDR_SIZE sizeof(struct iphdr)         //基础ip头的大小
#define IP_HDR_SIZE(hdr) (hdr->ihl * 4)               //整个ip头的大小
#define IP_DATA(hdr) ((char *)hdr + IP_HDR_SIZE(hdr)) // ip数据部分 char*类型
