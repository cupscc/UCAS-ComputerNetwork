...
// if is 1 make this port as the port as root port
static int compare_node(stp_port_t *p, struct stp_config *config)
{
	// u64 config_root_id = ntohll(config->root_id);
	u64 config_root_id = ntohll(config->root_id);
	if (config_root_id >= p->designated_root)
	{
		int config_root_path_cost = ntohl(config->root_path_cost);
		if (config_root_path_cost >= p->designated_cost)
		{
			u64 config_switch_id = ntohll(config->switch_id);
			if (config_switch_id >= p->designated_switch)
			{
				u16 config_port_id = ntohs(config->port_id);
				if (config_port_id >= p->designated_port)
				{
					return 0;
				}
				else
...
static int compare_port(stp_port_t *p1, stp_port_t *p2)
{
...
static void stp_handle_config_packet(stp_t *stp, stp_port_t *p, struct stp_config *config)
{
	if (compare_node(p, config))
	{
		...
		{
			p1 = &stp->ports[i];
			if (!stp_port_is_designated(p1))
			{
				int is_root_port = 0;
				for (int j = 0; j < stp->nports; j++)
				{
					stp_port_t *p2 = &stp->ports[j];
					if ((p1 != p2) && !stp_port_is_designated(p2))
					{
						if (!compare_port(p1, p2))
						{
							is_root_port = 1;
							break;
						}
		...
		{
			stp->root_port = p1;
			stp->designated_root = p1->designated_root;
			stp->root_path_cost = p1->designated_cost + p1->path_cost;
		}
		else
		{
			stp->root_port = NULL;
...
void stp_port_handle_packet(stp_port_t *p, char *packet, int pkt_len)
{
	...
	struct stp_header *header = (struct stp_header *)(packet + ETHER_HDR_SIZE + LLC_HDR_SIZE); //因为生成树是网络层的，前面有以太网头和LLC层头部信息
