...
void *handle_http_request(void *csock_in);
...
int main()
{
    ...
    while (1)
    {
        ...
        pthread_create(&thport_80, NULL, handle_http_request, (void *)csock_80);
...
void parse_url(Request_t *request)
{
    char str[50];
    char s[2] = "/";
    char *token;
    if (request->Url.url != NULL)
    {
        strcpy(str, request->Url.url);
    }
    request->Url.Protocol = strtok(str, s);
    request->Url.IP = strtok(NULL, s);
    request->Url.Path = (strrchr(request->Url.url, '/') + 1);
}
...
void find_url(char *str, Request_t *request)
{
    ...
    for (int i = 0, j = 0; i < start && j < 10; i++, j++)
    {
        request->Method[j] = str[i];
    }
    for (int i = start, j = 0; i <= end && j < 50; i++, j++)
    {
        request->Url.url[j] = str[i];
    }
}
void handle_https_request(SSL *ssl)
{
    ...
    else
    {
        ...
        Response_t response;
        FILE *fp = fopen(request->Url.Path, "r");
        if (fp == NULL)
        {
