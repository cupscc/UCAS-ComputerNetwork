...
{
	rt_entry_t *rt_dest = longest_prefix_match(dst);
	...
	if (rt_dest)
	{
		...
		if (iphdr->ttl <= 0)
		{
			log(DEBUG, "handle icmp ttl 0 packet\0");
		...
		else
		{
			...
			if (rt_dest->gw == 0)
			{
				iface_send_packet_by_arp(rt_dest->iface, dst, packet, len);
			}
			else
			{
